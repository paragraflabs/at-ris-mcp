 ...
 **Kern-Prinzipien:**
 ...
 - **v0.3:** Landesrecht (9 Bundesländer), `Sonstige` (Erlässe), `Bezirke`/`Gemeinden`. ✅ **erledigt** — *4 neue Tools (ris_search_state_law/misc/district/municipality); generisches Record-Mapping über alle Metadaten-Kategorien. Hinweis: `LrKons`-Bundeslandfilter braucht die gepunktete Form `Bundesland.SucheIn<Land>=true` (flache Form wird ignoriert).*
 - **v1.0:** Härtung, Doku, PyPI + MCP-Registry-Eintrag (falls Veröffentlichung gewählt), `Erv` (engl. Übersetzungen). ✅ **erledigt** — *Erv über `ris_search_law` (Applikation=Erv, SearchTerms/Title/Source); `Sonstige`-Feinfilter (Avsvnummer/Spgnummer/Sitzungsnummer etc., app-spezifisch); SPDX-Header + py.typed; gehärtete pyproject-Metadaten; GitHub-Actions-CI (Test-Matrix 3.11–3.13 + build/twine); CHANGELOG.md + server.json (MCP-Registry). Release-fertig (kein Upload durchgeführt).*
